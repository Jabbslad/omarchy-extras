...
A fourth package, `packaging/sc200pc-ipu75xa-config/`, carries vendor HAL assets (AIQB / graph binaries, derived from Windows). A fifth, `packaging/intel-ipu7-camera-sc200pc/`, carries patches (0001–0005) that fix pipeline construction in the vendor HAL. The vendor HAL path now successfully loads the Windows graph binary, runs 3A, and constructs the PSYS pipeline, but is **blocked on a per-graph hash mismatch**: the Windows graph binary's autogen data layout (hash `0xE7F37F28`) differs from the Linux HAL's compiled layout (hash `0x246C440B`). The container format is compatible (same header layout) but per-graph data layout differs from Linux autogen code. Resolving this requires Intel to generate a Linux-native graph binary with their proprietary graphspec compiler. The `install.sh` Galaxy Book6 block actively *removes* HAL-only WirePlumber overrides (`10-disable-libcamera.conf`, `60-hide-ipu7-v4l2.conf`) so the native libcamera path is preferred.
